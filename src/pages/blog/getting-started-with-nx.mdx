...
_**Note:** You can append the flat `--dry-run` to each command, to see what a command will do, without actual changes to your workspace._
...
_**Note:** You can run the command `yarn list @nrwl/react` to see all the things you can do with this plugin._
...
_**Note:** We can skip the `--base` flag since it will default to the main branch._
